...
const ArbScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetch('https://www.cosmicrms.com/api/ld-lf/arb-net-total')
      .then(response => response.json())
      .then(data => {
        setData([
          {
            GROUPNAME: 'GroupName',
            NetMTM_USD: 'NetMTM_USD',
            INRMTM: 'INRMTM',
            ARB: 'ARB',
          },
  ...
  const renderHeader = () => (
    <View style={styles.row}>
      <Text style={[styles.cell, styles.thead]}>GroupName</Text>
      <Text style={[styles.cell, styles.thead]}>NetMTM_USD</Text>
      <Text style={[styles.cell, styles.thead, {paddingLeft: '2%'}]}>
        INRMTM
      </Text>
      <Text style={[styles.cell, styles.thead, {paddingLeft: '3%'}]}>ARB</Text>
    </View>
  );
  ...
  return (
    <LinearGradient colors={['#780206', '#061161']} style={styles.container}>
      <Animated.View style={[styles.innerContainer, animatedStyle]}>
        <Text style={styles.title}>Arb Net Total</Text>
